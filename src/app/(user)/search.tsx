...
export default function SearchScreen() {
  ...
  return <View className="flex-1 gap-3 bg-background p-4"><View className="mx-auto w-full max-w-2xl flex-row items-center gap-2"><Button size="icon" variant="ghost" onPress={() => goBack('/(user)')}><Icon as={ArrowLeft} /></Button><Input autoFocus className="flex-1" placeholder="Search coffee shops…" value={search} onChangeText={(value) => setFilter('search', value)} /></View><View className="mx-auto w-full max-w-2xl"><DiscoveryFilterRow /><Text className="mt-2 text-sm text-muted-foreground">{loading ? 'Loading coffee shops…' : `${shops.length} coffee shop${shops.length === 1 ? '' : 's'} found`}</Text></View>{loading ? <View className="mx-auto w-full max-w-2xl gap-3"><ShopCardSkeleton /><ShopCardSkeleton /><ShopCardSkeleton /></View> : <FlatList data={shops} keyExtractor={(shop) => shop.id} contentContainerClassName="mx-auto w-full max-w-2xl gap-3 pb-4" ListEmptyComponent={<Text className="py-8 text-center text-muted-foreground">No approved coffee shops match these filters yet.</Text>} renderItem={({ item }) => <ShopCard shop={item} onPress={() => router.push({ pathname: '/(user)/shop/[id]', params: { id: item.id } })} saved={savedShopIds.includes(item.id)} saving={savingShopId === item.id} onToggleSaved={() => void toggleSavedShop(item.id)} compared={ids.includes(item.id)} onToggleCompare={() => toggle(item.id)} />} />}</View>;
}
