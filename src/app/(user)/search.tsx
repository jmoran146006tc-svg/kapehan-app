...
export default function SearchScreen() {
  ...
  return <View className="flex-1 gap-3 bg-background p-4"><View className="flex-row items-center gap-2"><Button size="icon" variant="ghost" onPress={() => router.back()}><Icon as={ArrowLeft} /></Button><Input autoFocus className="flex-1" placeholder="Search coffee shops…" value={search} onChangeText={(value) => setFilter('search', value)} /></View><DiscoveryFilterRow /><Text className="text-sm text-muted-foreground">{shops.length} coffee shop{shops.length === 1 ? '' : 's'} found</Text>{savedShopIds.length === 0 ? null : null}<FlatList data={shops} keyExtractor={(shop) => shop.id} contentContainerClassName="gap-3 pb-4" renderItem={({ item }) => <ShopCard shop={item} onPress={() => router.push({ pathname: '/(user)/shop/[id]', params: { id: item.id } })} saved={savedShopIds.includes(item.id)} saving={savingShopId === item.id} onToggleSaved={() => void toggleSavedShop(item.id)} compared={ids.includes(item.id)} onToggleCompare={() => toggle(item.id)} />} />{ids.length >= 2 ? <Button className="absolute right-4 shadow-lg" style={{ bottom: BottomTabInset + 16 }} onPress={() => router.push('/(user)/compare')}><Text>Compare ({ids.length}) →</Text></Button> : null}</View>;
}
