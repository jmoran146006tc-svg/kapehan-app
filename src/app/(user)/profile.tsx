...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (!user) return;
    return onSnapshot(query(collectionGroup(db, 'reviews'), where('userId', '==', user.uid)), (snapshot) => setReviewCount(snapshot.size));
  }, [user]);
  ...
  return (
    <ScrollView className="flex-1 bg-background" contentContainerClassName="gap-5 pb-8">
      ...
      <View className="gap-5 px-4">
        ...
        <View className="gap-3"><Text className="text-xl font-bold">Search Preferences</Text><Controller control={control} name="wifiOnly" render={({ field }) => <View className="flex-row gap-2"><Button className="flex-1" variant={field.value ? 'default' : 'outline'} onPress={() => field.onChange(true)}><Text>WiFi</Text></Button><Button className="flex-1" variant={!field.value ? 'default' : 'outline'} onPress={() => field.onChange(false)}><Text>No preference</Text></Button></View>} /><Controller control={control} name="priceBuckets" render={({ field }) => <View className="gap-2"><Text className="font-semibold">Price tier</Text><View className="flex-row flex-wrap gap-2">{(['budget', 'moderate', 'premium'] as PriceBucket[]).map((bucket) => <FilterChip key={bucket} label={PRICE_BUCKET_LABELS[bucket]} selected={field.value.includes(bucket)} onPress={() => field.onChange(field.value.includes(bucket) ? field.value.filter((value) => value !== bucket) : [...field.value, bucket])} />)}</View></View>} /><Controller control={control} name="tags" render={({ field }) => <View className="gap-2"><Text className="font-semibold">Favorite features</Text><View className="flex-row flex-wrap gap-2">{TAG_OPTIONS.map((tag) => <FilterChip key={tag} label={tag} selected={field.value.includes(tag)} onPress={() => field.onChange(field.value.includes(tag) ? field.value.filter((value) => value !== tag) : [...field.value, tag])} />)}</View></View>} /><Controller control={control} name="openNowOnly" render={({ field }) => <Button variant={field.value ? 'default' : 'outline'} onPress={() => field.onChange(!field.value)}><Text>{field.value ? 'Open now only' : 'Include closed shops'}</Text></Button>} /><Button loading={isSaving} loadingLabel="Saving…" onPress={handleSubmit(savePreferences)}><Text>Save preferences</Text></Button>{saveError ? <Text accessibilityRole="alert" className="text-destructive">{saveError}</Text> : null}</View>
      </View>
    </ScrollView>
  );
}
