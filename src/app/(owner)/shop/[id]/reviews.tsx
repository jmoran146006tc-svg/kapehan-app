...
function OwnerReviewsContent({ shop }: { shop: Shop }) {
  ...
  async function reply(review: Review) {
    ...
    try {
      ...
      setError(getUserFriendlyError(replyError, replyError instanceof Error ? replyError.message : 'We could not send that reply.'));
    } finally {
      setSubmitting(null);
    }
  }

  return <ScrollView className="flex-1 bg-background px-4" contentContainerClassName="mx-auto w-full max-w-2xl gap-4 py-5 pb-8">
    <Card><CardHeader className="flex-row gap-5"><View className="items-center justify-center"><Text className="text-4xl font-bold">{shop.avgRating.toFixed(1)}</Text><Icon as={Star} size={20} fill="currentColor" className="text-accent" /></View><View className="flex-1 gap-1">{[5, 4, 3, 2, 1].map((rating) => <View key={rating} className="flex-row items-center gap-2"><View className="w-5 flex-row items-center gap-0.5"><Text className="text-xs">{rating}</Text><Icon as={Star} size={10} fill="currentColor" className="text-accent" /></View><View className="h-2 flex-1 overflow-hidden rounded-full bg-secondary"><View className="h-full bg-accent" style={{ width: `${total ? (counts[String(rating) as keyof typeof counts] / total) * 100 : 0}%` }} /></View><Text className="w-8 text-right text-xs text-muted-foreground">{total ? Math.round((counts[String(rating) as keyof typeof counts] / total) * 100) : 0}%</Text></View>)}</View></CardHeader></Card>
