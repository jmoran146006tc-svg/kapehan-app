...
function OwnerMenuContent({ shop }: { shop: Shop }) {
  ...
  return <>
    <ScrollView className="flex-1 bg-background px-4" contentContainerClassName="mx-auto w-full max-w-2xl gap-4 py-5 pb-8">
      ...
      {PRODUCT_CATEGORIES.map((category) => { const categoryProducts = products.filter((product) => product.category === category); if (!categoryProducts.length) return null; return <View key={category} className="gap-2"><Text className="text-sm font-bold tracking-wider text-muted-foreground">{category.toUpperCase()}</Text>{categoryProducts.map((product) => <Card key={product.id} className="py-3"><CardHeader className="min-w-0"><View className="w-full min-w-0 flex-row items-center gap-3"><View className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-secondary">{product.photoUrl ? <Image source={{ uri: product.photoUrl }} className="h-full w-full" /> : null}</View><Button variant="ghost" className={cn('flex-1 min-w-0 shrink items-start px-0', Platform.select({ web: 'hover:bg-secondary/60 active:bg-secondary/70' }))} onPress={() => { setAdding(false); setEditing(product); }}><View className="flex-1 min-w-0 shrink"><CardTitle numberOfLines={1} ellipsizeMode="tail" className="min-w-0 shrink">{product.name}</CardTitle>{product.description ? <CardDescription numberOfLines={2} ellipsizeMode="tail" className="min-w-0 shrink">{product.description}</CardDescription> : null}<Text numberOfLines={1} ellipsizeMode="tail" className="mt-1 min-w-0 shrink font-bold">₱{product.price.toLocaleString()}</Text></View></Button><Button size="sm" variant="outline" className="shrink-0" onPress={() => void removeProduct(product)}><Text>Remove</Text></Button></View></CardHeader></Card>)}</View>; })}
